...
服务启动时会自己定白平衡、曝光、增益，约 30 秒，日志里能看到每步的结果：

```
🎚️ 自动标定相机…
   曝光 39（背景高光 194/235，32 fps）
   增益 0（保持最小，背景高光 194，够检测用，不引入噪点）
   白平衡 4914K（背景偏色 0.056，0 为中性）
   注意：S1=18% 有大片死白，若非纯白页面则花屏检测在该屏失效
```

`--no-camera-calib` 关掉。几个不显然的设计取舍：

| 取舍 | 为什么 |
| --- | --- |
| 基准取**背景**（画面扣掉各屏 ROI），不取屏幕内容 | 屏幕自发光且内容一直在变。实测增益 0/128/254 三档各屏死白是 65%/15%/58%，**非单调**——中位数被内容带着走，二分建立在这种判据上必然收敛到垃圾值 |
| 先把增益压到最低，再用曝光够亮度 | 同样亮度下噪声只跟增益走。反过来做会选出"极短曝光 + 最大增益"，实测落到曝光 20 / 增益 254，画面糊成一片噪点 |
| 曝光同时受**帧率**约束，而不是写死曝光上界 | 拉长曝光既推高亮度也压低帧率，两者同向，二分仍成立，且不依赖机型。低于 20fps 闪屏/冻屏判定就不可靠 |
| 增益补光门槛很高（背景高光低于目标一半才补），且限幅到量程一半 | 噪声是永久代价，背景偏暗却不影响检测。台架上屏幕会自己亮灭，补光一旦触发，下次屏幕全亮时就变成过曝加噪点 |
| 白平衡排在亮度之后 | 评中性看的是背景像素的通道比例，背景一旦死白三个通道都钳在 255，比例恒等于 1，看着"完美中性"其实是没信息。实测放前面选出 3328K（偏色 0.044），放后面选出 4385K（偏色 0.021） |
| 各屏死白只报告、不参与搜索 | 一块屏正在显示纯白页面时读数本就该接近满值，那是真内容不是过曝 |

**调参前先把屏幕钉亮。** 台架若装在暗机柜里，**屏幕本身就是背景的主要光源**——屏幕
一亮一灭，整幅画面的亮度跟着跳，任何测量都不可复现。实测未钉屏时同一组参数测出的
背景均值在 50 和 155 之间来回跳，还出现"曝光调小反而更亮"这种非单调结果。先跑
`adb shell svc power stayon true` 再调。

**自动曝光未必比手动差，先实测再定。** 该模组在暗机柜里实测：自动曝光 30fps 下背景
就有 110~155，而**任何手动组合都达不到**（手动最好是曝光 1200 + 增益 20 得 92，还掉到
20fps）。

> 坑：**自动模式下 `exposure_time_absolute` 回报的是假值。** 同样读到 450，自动模式下
> 背景是 155，手动写 450 只有 54。别拿这个读数当依据，更别拿它去"对齐"另一台机器。

**噪点只跟增益走，亮度交给曝光。** 实测增益 0/30/112 对应时域噪声 1.12/1.32/1.59
（时域噪声 = 连续帧在静态背景上的逐像素差；空间 std 分不开纹理和噪点）。自动曝光会
自己补亮度，所以压增益不会让画面变暗。

**锐化是划算的。** `sharpness` 0→7 清晰度 267→829，噪声 1.32→3.29，清晰度/噪声比
反而从 202 升到 252——是真加细节，不是放大噪点。

**换个做法：对齐到另一台已经调好的台架。** 同型号相机可以直接搬参数，用
`--no-camera-calib` 关掉标定，在 `ExecStartPost` 里写死。但**只搬"开自动"是没用的**：
自动曝光按场景测光，环境不同结果就不同。实测采集端台架在明亮房间里，自动曝光落在
`625`；远端台架在暗机柜里，同样开自动会跑到 `10000`，把屏幕推到 92~100% 死白。
要对齐的是自动曝光实际落到的**值**，不是模式。

> `ExecStartPost` 的 `sleep` 必须落在服务打开相机**之后**：内核在设备 open 时会把
> 控制项复位成默认。实测 `sleep 8` 会赶在启动诊断完成前写入，参数静默丢失，表现为
> "单元里写了值但读回来是别的"。给到 25 秒以上。

读运行中相机的控制项（服务独占设备时 `v4l2-ctl` 也能读，无 `v4l-utils` 时可走 ioctl）：

```bash
v4l2-ctl -d /dev/video0 --list-ctrls
```

**已知限制**：标定拿背景当基准，前提是背景由环境光照亮。台架装在暗机柜里时**屏幕
才是背景的主要光源**，这个前提不成立，标定会把画面压得过暗——那种台架用
`--no-camera-calib` 关掉，按上面的方法手工定参数。

另外，标定只看启动那一刻的画面。台架上屏幕会自己亮灭，若标定时有屏是熄的，
选出的曝光没考虑它亮起后的高光。屏幕状态变化大时重启服务重标一次。
...
| **各类型实时分数** | 「屏幕状态」表里每块屏逐类型列出当前分数（`BLACK 0.00 / GARBLE 0.00 / FLICK 0.00`），**正常时也显示**，一眼看出离阈值还有多远；命中的类型标红。鼠标悬停看类型全名与检测详情 |
...
| 类型 | 代号 | 画面配色 | 说明 |
|---|---|---|---|
| `black_screen` | BLACK | 红 | 默认开启。屏幕几乎全黑但**亮区在动**（开机动画、加载图标）时判为正常 —— 面板还在出画面，不算故障 |
...
网页上用 `D1 / D2 / D3` 按钮切换要看哪块屏，**只有选中的那块在采集**，其余完全不占
设备与主机资源。这样做有两个原因：

1. **并发会串台。** 实测这类车机不能同时对多块屏跑 `screenrecord --display-id`：

   | 组合 | 结果 |
   |---|---|
   | 单独一路 | 三块屏都正确 |
   | IVI + Cluster / IVI + Rear | 正确 |
   | Cluster + Rear | **串台** |
   | 三路并发 | **串台**（降到 640 长边 / 1M 码率反而更糟） |

2. **并发拖低帧率。** 多路 H.264 解码 + JPEG 编码同时跑，谁都快不了。

单路独占后**任意一块屏都能跑满帧率**，包括原先只能走 screencap 的那块：

| | 三路同时渲染 | 单路可切换 |
|---|---|---|
| D1 IVI | 0.7 fps (screencap) | **18.5 fps** |
| D2 Cluster | 1.1 fps (screencap) | **22.3 fps** |
| D3 Rear | 0.3 fps (screencap) | **22.4 fps** |

切换后约 5 秒出画（FFmpeg 探测裸流的固有延迟），期间显示"连接中…"。

**串台判据是相对比较**：screenrecord 有数秒编码延迟，与瞬时 screencap 天然对不齐，
屏上一有动画，来源正确时相关度也可能只有 0.2~0.3。早先用绝对阈值 0.40，把来源
全部正确的流逐个误判成串台、全降到 0.3~1.1fps。现在只有"更像另一块屏"
（超出自身相关度 `FALLBACK_MARGIN`）才判定回落。

> 若哪天遇到能并发的设备，`--device-record-max` 仍保留（默认 2），但当前 UI 单路
> 渲染下只会用到一路。

**远程操作**：投屏画面上可直接操作设备——**单击 = 点按，拖动 = 滑动**。
点过画面之后键盘输入会转发到设备（回车 / 退格发对应按键），点页面其它地方即交还键盘。
坐标按归一化 0~1 传给服务端再乘该屏原生分辨率，浏览器里画面缩放不影响落点。
...
| GET | `/api/status` | 分辨率、fps、各屏状态、当前 ROI；每块屏带 `scores{类型: {score, abnormal, info}}`，正常类型也在内 |
...
| 屏幕几乎全黑但角落有动画图标，被判 BLACK | 只按暗像素占比判，99.8% 暗就够触发 | 已修：亮区存在（>0.15% 像素超阈）**且在动**时豁免。只看亮度不行——桌面反光同样亮且面积相近，但反光不会动；动画有停顿帧，故动过一次保持 2 秒 |
...
| 屏幕看着发白/发蓝，灰度死白占比却是 0%，GARBLE 测不出 | **某个通道单独被推爆**。灰度指标查不出这种丢失：实测某台架 ROI 灰度死白 0.0%，蓝通道却有 ~20% 钳在 255 | 排查看**逐通道**死白占比。服务启动时会自动标定（见下），异常时先看启动日志里的标定结果；要手动压就降 `gain`（等比缩放各通道，不动色彩）。**别用白平衡去压**——WB 温度调高确实能降蓝通道，但那是拿偏色换死白（6500 时中性面 `B/G` 掉到 0.838，画面明显偏暖） |
| 画面整体偏暖/偏冷 | 自动白平衡在漂，或 WB 温度被调离中性档 | 找中性档：对着画面里一块**非自发光的白/灰参照物**（台面、机柜板）扫 WB 温度，取 `B/G` 与 `R/G` 都最接近 1.000 的值固定下来。该模组实测 4600（即默认值）最中性 |
| 相机对不上焦，画面糊 | 模组是定焦的 | `v4l2-ctl --list-ctrls` 若无任何 focus 项则软件调不了，只能现场旋转镜筒。注意 `zoom_absolute` 是变焦不是对焦 |
