...
| 屏幕看着发白/发蓝，灰度死白占比却是 0%，GARBLE 测不出 | **某个通道单独被推爆**。灰度指标查不出这种丢失：实测某台架 ROI 灰度死白 0.0%，蓝通道却有 ~20% 钳在 255 | 排查看**逐通道**死白占比。修法是降 `gain`（等比缩放各通道，不动色彩）：`v4l2-ctl -d /dev/video0 -c gain=30`，实测蓝死白 20.2%→2.1%。**别用白平衡去压**——把 WB 温度调高确实能降蓝通道，但那是拿偏色换死白（6500 时中性面 `B/G` 掉到 0.838，画面明显偏暖） |
