...
| 屏幕看着发白/发蓝，灰度死白占比却是 0%，GARBLE 测不出 | **自动白平衡把单个通道推爆**。灰度指标查不出这种丢失：实测某台架 S2/S3 灰度死白 0.0%，蓝通道各有 ~20% 钳在 255 | 关自动白平衡并固定色温：`v4l2-ctl -d /dev/video0 -c white_balance_automatic=0,white_balance_temperature=6500`。WB 温度对蓝通道单调可调（2800→82% 死白，6500→0%）。排查要看**逐通道**死白占比，别只看灰度 |
